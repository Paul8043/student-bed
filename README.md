...
The Internal-Model-Specification, for short IMS, covers the aspects of the design and implementation. It holds all the details from the view of a cabinetmaker (that's my role). This document can be found here [IMS](docs/IMS.md).
...
With the 3D-model ready, it should be a simple task to generate the SVG-files. But this was not true.

The generation of SVG-files was more difficult than expected. The original idea was to make some sectional views of the needed parts. My router expects SVG-files and CadQuery can generate SVG-files and many other formats. This works in principle, however there are two limitations:
1) A simple sectional view is not enough for the router. Some helper-lines are needed, which have to be added.
...
# Mile-Stone #13: Implementation

The model does not show all details. The prototype generated by the 3D-printer has some special requirements, and this may deviate from the wooden realization. All the aspects, how the wooden parts are joined together are not covered in the model. These are decisions of the implementation and the issues in this area are described in the [IMS](docs/IMS.md). The bed is a simple construction and therefore the IMS can be kept short. Only a few things need to be discussed there. The frame, consisting of jambs and ribs, does not need any kind of special junction-elements. The notches in the wood are sufficient. For the duckboard it looks different. The bed has to be demountable and because of this some screws are a good and simple solution. I have chosen screws with metrical threads and these screws need a metallic counterpart, called inserts. This gives a very stable junction, which can be easily detached. The leftover is normal wood-working. The focus here is, how the wooden parts can be routed easily. This depends on the equipment, you can use. The drawings for routing are already covered in the previous mile-stone.

After so much work we can have a look at the real bed!

![bed-wood-final](docs/bed-wood-final.jpg)
